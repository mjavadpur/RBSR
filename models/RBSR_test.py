...
class RBSRNet(nn.Module):
    ...
    def burst_propagate(self, feats, flows, module_name, feat_base):
        n, _, _, h, w = flows.size()
        frame_idx = range(0, len(feats['spatial']))  #
    ...
    def forward(self, lqs):
        ...
        feats['spatial'] = [feats_[:, i, :, :, :] for i in range(t-1, -1, -1)]
        base_feat = feats_[:, 0, :, :, :]
        module = 'backward_1'
        feats[module] = []
        feats = self.burst_propagate(feats, flows_backward, module, base_feat)
